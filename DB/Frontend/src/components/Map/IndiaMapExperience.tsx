...
import INDIA_OFFICIAL_GEO from '../../data/indiaStatesOfficialGeo.json';
...
export const IndiaMapExperience: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  const [data, setData] = useState<any>(INDIA_OFFICIAL_GEO);
  ...
  // ViewBox: 940 x 900 -> Tightly bounds India mainland and Northeast with maximum size and zero distortion
  const { projection, path } = useMemo(() => {
    if (!data || !data.features || data.features.length === 0) {
      const p = geoMercator().center([82.8, 22.5]).scale(1150).translate([470, 450]);
  ...
  const resolveStateId = (name: string): string => {
    ...
    if (n.includes('lakshadweep') || n.includes('laccadive')) return 'lakshadweep';
    if (n.includes('andaman') || n.includes('nicobar')) return 'andaman-and-nicobar';
    ...
    if (n.includes('madhyapradesh')) return 'madhya-pradesh';
    if (n.includes('uttarpradesh')) return 'uttar-pradesh';
    if (n.includes('ladakh')) return 'ladakh';
    if (n.includes('kashmir')) return 'jammu-and-kashmir';
    return '';
  };
  ...
  return (
    <div className="w-full relative select-none" aria-label="Interactive Map of India">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 xl:gap-8 items-stretch">
        ...
        {/* ============================================================ */}
        <div className="lg:col-span-4 xl:col-span-3 flex flex-col justify-between space-y-4 max-w-full lg:max-w-[420px]">
          ...
          <div className="space-y-2">
            <h2 className="font-serif text-2xl sm:text-3xl xl:text-[2.2rem] font-bold text-parchment-950 leading-[1.15] tracking-tight">
              {t('mapSection.title') || 'Interactive India Map'}
            </h2>

            <p className="text-xs sm:text-sm text-parchment-700 font-sans leading-relaxed">
              {t('mapSection.subtitle') || 'Select any state boundary to zoom into its verified living archives. Explore Maharashtra as the primary core hub, alongside demonstration hubs in Assam and Meghalaya.'}
            </p>
          </div>
        ...
        <div className="lg:col-span-8 xl:col-span-9 w-full">
          <div className="india-canvas-wrapper relative w-full h-[500px] sm:h-[580px] md:h-[660px] lg:h-[78vh] xl:h-[84vh] min-h-[460px] max-h-[920px] rounded-3xl overflow-hidden bg-gradient-to-br from-[#FAF7F2] via-[#F4EDE4] to-[#EAE1D4] border border-parchment-300/80 shadow-heritage-lg flex items-center justify-center p-1 sm:p-3">
            ...
            {/* Top-Right Reset Button (Only shown when a state is selected) */}
            {selectedState && (
              <div className="absolute top-3 sm:top-4 right-3 sm:right-4 z-10 pointer-events-auto">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-white/90 hover:bg-white backdrop-blur-md border border-parchment-300 text-xs font-semibold text-parchment-800 transition-colors shadow-heritage-xs"
                  title="Clear selection"
                >
                  <RotateCcw className="w-3.5 h-3.5 text-category-temples" />
                  <span>{t('mapSection.resetView') || 'Reset View'}</span>
                </button>
              </div>
            )}
                ...
                >
                  ...
                  <g className="india-state-fill-layer">
                    {data.features.map((f: any, i: number) => {
                      ...
                      const isIsland = norm(n).includes('lakshadweep') || norm(n).includes('andaman');
                      ...
                      return (
                        <path
                          ...
                          stroke={isIsland ? fillColor : 'none'}
