...
export const StateInteractiveMap: React.FC<StateInteractiveMapProps> = ({
  ...
  const targetFeature = useMemo(() => {
    return INDIA_OFFICIAL_GEO.features?.find((f: any) => 
      norm(featureName(f)).includes(config.nameFilter)
    ) || null;
  }, [config.nameFilter]);

  const [feature, setFeature] = useState<any>(targetFeature);
  const [loading, setLoading] = useState(false);
  const [hoveredItem, setHoveredItem] = useState<CulturalItem | null>(null);

  useEffect(() => {
    const target = INDIA_OFFICIAL_GEO.features?.find((f: any) => 
      norm(featureName(f)).includes(config.nameFilter)
    ) || null;
    setFeature(target);
    setLoading(false);
