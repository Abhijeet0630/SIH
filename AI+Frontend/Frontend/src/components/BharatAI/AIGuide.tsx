...
/**
 * Parses inline Markdown constructs including **bold** and [link](url) syntax cleanly into React nodes.
 */
const formatInlineStyles = (textStr: string, isUserMessage: boolean): React.ReactNode[] => {
  const tokenRegex = /(\*\*.*?\*\*|\[.*?\]\(.*?\))/g;
  ...
  return tokens.map((token, i) => {
    ...
    if (linkMatch) {
      ...
      const isInternal = targetUrl.startsWith('/') || targetUrl.includes('/item/') || targetUrl.includes('/monument/') || targetUrl.includes('/state/');
      ...
      if (isInternal) {
        ...
        return (
          <Link
            key={i}
            to={path}
            className="font-bold text-amber-800 underline hover:text-amber-950 transition-colors inline-flex items-center gap-0.5"
          >
            <span>{label}</span>
          </Link>
        );
      ...
      return (
        <a
          ...
          className="font-bold text-amber-800 underline hover:text-amber-950 transition-colors"
        >
          {label}
        </a>
      );
...
/**
 * Renders structured multi-line Markdown text into styled React elements.
 */
    ...
    <div className="space-y-1 leading-relaxed">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('### ')) {
          return (
            <h4 key={idx} className={`font-bold text-xs sm:text-sm mt-2 mb-0.5 ${isUserMessage ? 'text-white' : 'text-amber-900'}`}>
        ...
        if (trimmed.startsWith('* ') || trimmed.startsWith('- ')) {
          return (
            <div key={idx} className="flex items-start gap-1.5 pl-1.5 text-xs sm:text-sm my-0.5">
              <span className={`font-bold shrink-0 ${isUserMessage ? 'text-white' : 'text-amber-700'}`}>•</span>
        ...
        if (numMatch) {
          return (
            <div key={idx} className="flex items-start gap-1.5 pl-1 font-medium text-xs sm:text-sm mt-1">
              <span className={`font-bold shrink-0 ${isUserMessage ? 'text-white' : 'text-amber-700'}`}>{numMatch[1]}</span>
...
export const AIGuide: React.FC = () => {
  ...
  const [currentAvatarState, setCurrentAvatarState] = useState<'idle' | 'thinking' | 'speaking' | 'happy' | 'curious' | 'surprised' | 'excited'>('idle');
