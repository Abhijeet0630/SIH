...
import { Menu, X, Globe, Compass, Sparkles, CalendarDays, Layers, Info } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-md border-b border-parchment-200/80 transition-colors shadow-heritage-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between gap-4">
        ...
        >
          ...
          <div className="min-w-0">
            <span className="block text-[9px] sm:text-[10px] uppercase font-bold tracking-widest text-category-temples truncate">
              Student Innovation · Heritage
            </span>
            <span className="block font-bold text-sm sm:text-base text-parchment-950 tracking-tight truncate group-hover:text-category-temples transition-colors">
              [ PROJECT NAME ]
            </span>
          </div>
        </Link>
        ...
        {/* Nav Tools (Language + Mobile Toggle) */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          
          {/* Language Selector */}
          <div className="relative flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-parchment-100 border border-parchment-200 shadow-heritage-xs">
            <Globe className="w-3.5 h-3.5 text-category-temples shrink-0" />
            <select
              aria-label="Select Language"
              value={language}
              onChange={(e) => setLanguage(e.target.value as LanguageCode)}
              className="bg-transparent text-xs font-semibold text-parchment-800 focus:outline-none cursor-pointer pr-1"
            >
              {availableLanguages.map((lang) => (
                <option key={lang.code} value={lang.code} className="bg-white text-parchment-900">
                  {lang.nativeLabel}
                </option>
              ))}
            </select>
