...
import meghalayaGeoData from '../../data/meghalayaGeo.json';
import meghalayaOfficialDistricts from '../../data/meghalayaOfficialDistricts.json';
...
export const MEGHALAYA_MAP_CATEGORIES: CategoryFilterOption[] = [
  {
    id: 'all',
    label: 'All Heritage',
    shortLabel: 'All',
    ...
    description: 'All verified cultural landmarks across the Khasi, Garo, and Jaintia Hills',
    categories: ['food', 'fashion', 'forts', 'temples', 'monuments', 'architecture', 'dance', 'music', 'crafts', 'tribal', 'languages', 'festivals', 'culture']
  },
  {
    id: 'bio_arch',
    label: 'Living Root Bridges',
    shortLabel: 'Root Bridges',
    icon: Trees,
    accentColor: '#15803D', // green-700
    description: 'Jingkieng Jri botanical engineering across Sohra and Nongriat subtropical rainforests',
    categories: ['architecture', 'monuments', 'culture']
  },
  {
    id: 'sacred_groves',
    label: 'Sacred Groves & Lore',
    shortLabel: 'Sacred Groves',
    icon: Landmark,
    accentColor: '#0F766E', // teal-700
    description: 'Law Kyntang untouched primordial forests, monolith alignments & clan sanctuaries',
    categories: ['tribal', 'culture']
  },
  {
    id: 'dance',
    label: 'Khasi & Garo Festivals',
    shortLabel: 'Dance & Lore',
    icon: Activity,
    accentColor: '#BE123C', // rose-700
    description: 'Shad Suk Mynsiem thanksgiving dances, Wangala 100-drum harvest beats & Ka Pomblang',
    categories: ['dance', 'music', 'festivals']
  },
  {
    id: 'food',
    label: 'Indigenous Cuisine',
    shortLabel: 'Cuisine',
    icon: UtensilsCrossed,
    accentColor: '#C2410C', // orange-700
    description: 'Khasi Jadoh rice infused with pork broths, Tungrymbai, and fermented sesame pastes',
    categories: ['food']
  },
  {
    id: 'crafts',
    ...
    description: 'Ryndia organic Eri peace silk and fine bamboo basketry of the hill tribes',
...
export const formatDistrictName = (name: string): string => {
  ...
  if (clean.toLowerCase() === 'east khasi hills') return 'East Khasi Hills';
  if (clean.toLowerCase() === 'west khasi hills') return 'West Khasi Hills';
  if (clean.toLowerCase() === 'west garo hills') return 'West Garo Hills';
  if (clean.toLowerCase() === 'east garo hills') return 'East Garo Hills';
  if (clean.toLowerCase() === 'south garo hills') return 'South Garo Hills';
  if (clean.toLowerCase() === 'jaintia hills') return 'Jaintia Hills';
  if (clean.toLowerCase() === 'ri bhoi') return 'Ri-Bhoi';
  return clean;
};

// Data-driven geographic centers for Meghalaya's primary hill regions
export const MEGHALAYA_REGION_CENTROIDS: Record<string, { name: string; shortLabel: string; lng: number; lat: number }> = {
  garo_hills: {
    name: 'GARO HILLS',
    shortLabel: 'Garo Hills',
    lng: 90.40,
    lat: 25.55
  },
  khasi_hills: {
    name: 'KHASI HILLS',
    shortLabel: 'Khasi Hills',
    lng: 91.45,
    lat: 25.55
  },
  jaintia_hills: {
    name: 'JAINTIA HILLS',
    shortLabel: 'Jaintia Hills',
    lng: 92.35,
    lat: 25.45
  }
};

export interface MeghalayaAtlasMapProps {
  items: CulturalItem[];
  selectedId?: string;
...
export const MeghalayaAtlasMap: React.FC<MeghalayaAtlasMapProps> = ({
  items,
  selectedId,
  onSelect,
  activeSubRegion = 'all',
  ...
  // Natural Geographic Dimensions (2.48:1 aspect ratio matching Meghalaya's wide mountain ridge)
  const MAP_WIDTH = 760;
  const MAP_HEIGHT = 420;

  // D3 Projection fitted precisely to authoritative Meghalaya district boundary collection
  const { projection, pathGenerator } = useMemo(() => {
    const proj = geoMercator();
    proj.fitExtent([[35, 30], [MAP_WIDTH - 35, MAP_HEIGHT - 30]], meghalayaOfficialDistricts as any);
    return {
      projection: proj,
  ...
  const categoryFilteredItems = useMemo(() => {
    const catConfig = MEGHALAYA_MAP_CATEGORIES.find(c => c.id === activeCategory);
  ...
  const getCategoryColor = useCallback((category: CulturalCategoryId): string => {
    switch (category) {
      case 'architecture':
      case 'monuments': return '#15803D'; // green-700 (living root bridges)
      case 'tribal': return '#0F766E'; // teal-700 (sacred groves)
      case 'dance':
      case 'festivals': return '#BE123C'; // rose-700 (harvest dances)
      case 'food': return '#C2410C'; // orange-700
      case 'crafts':
      case 'fashion': return '#7E22CE'; // purple-700
  ...
  const projectedRegions = useMemo(() => {
    return Object.entries(MEGHALAYA_REGION_CENTROIDS).map(([id, reg]) => {
  ...
  // Dynamic Geographic Scale Calculation (calibrated at 25.5°N latitude across Meghalaya)
  const scaleInfo = useMemo(() => {
    const kmPerPixel = 0.44 / zoomLevel;
  ...
  return (
    <div className={`meghalaya-atlas-container relative bg-parchment-50 rounded-3xl border border-parchment-300/80 shadow-heritage-md overflow-hidden select-none ${className}`}>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-parchment-500 hidden sm:block">
              High-altitude plateau boundaries, living root bridges & sacred groves.
            </p>
          </div>
        </div>
      ...
      <div className="category-filter-rail px-4 sm:px-6 py-2 bg-parchment-100/60 border-b border-parchment-200/80 overflow-x-auto no-scrollbar flex items-center gap-1.5 z-20 relative">
        ...
        {MEGHALAYA_MAP_CATEGORIES.map(cat => {
          const Icon = cat.icon;
      ...
      >
        ...
        <svg
          ...
          role="img"
          aria-label="Authoritative Meghalaya Cultural Atlas Map with real administrative districts"
        >
          <defs>
            {/* Rich Topographic Landmass Gradient */}
            <radialGradient id="meghalayaLandGrad" cx="50%" cy="45%" r="65%">
            ...
            <filter id="activeMarkerGlow" x="-60%" y="-60%" width="220%" height="220%">
              <feDropShadow dx="0" dy="3" stdDeviation="5" floodColor="#15803D" floodOpacity="0.5" />
            </filter>
          </defs>
          ...
          >
            {/* 1. Base Meghalaya State Landmass Polygon with Elevation Shadow */}
            <path
              d={pathGenerator(meghalayaGeoData as any) || ''}
              fill="url(#meghalayaLandGrad)"
            ...
            <g className="district-polygons">
              {meghalayaOfficialDistricts.features.map((feature: any, index: number) => {
            ...
            <g className={`geographic-zones pointer-events-none select-none font-serif font-bold text-[#78350F] transition-opacity duration-300 ${zoomLevel > 2.2 ? 'opacity-15' : 'opacity-30'}`}>
              {projectedRegions.map(reg => (
                <text
                  ...
                  y={reg.y}
                  textAnchor="middle"
                  className="text-[12px] sm:text-[13px] tracking-widest"
                >
                  {reg.name}
                </text>
              ))}
      ...
      <div className="atlas-footer px-4 sm:px-6 py-2.5 bg-white/90 backdrop-blur-md border-t border-parchment-200 flex flex-wrap items-center justify-between gap-2 text-xs z-20 relative">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          ...
          {[
            ...
            { id: 'khasi-hills', label: 'Khasi Hills' },
            { id: 'garo-hills', label: 'Garo Hills' },
            { id: 'jaintia-hills', label: 'Jaintia Hills' }
          ].map(div => (
            <button
              key={div.id}
              onClick={() => {
        ...
        <div className="text-[11px] text-parchment-500 hidden lg:flex items-center gap-3">
          ...
          <span>Click pins for botanical & tribal lore</span>
        </div>
      </div>
    </div>
  );
